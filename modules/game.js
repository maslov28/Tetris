...
export class Game {
  ...
  createTetramino() {
    const keys = Object.keys(tetraminoes);  // массив букв
    const letterTetramino = keys[Math.floor(Math.random() * keys.length)]; // рандомная буква
    const rotation = tetraminoes[letterTetramino]; // рандомная фигура
    const rotationIndex = Math.floor(Math.random() * rotation.length); // индекс поворота
    const block = rotation[rotationIndex]; // рандомно повернутая фигура
    console.log(block);
